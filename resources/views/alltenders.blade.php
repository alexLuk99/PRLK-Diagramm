                    ...
                    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                            <ul class="navbar-nav">
                                    ...
                                    <li class="nav-item active">
                                             <a class="nav-link" href="alltenders">AnonymousTenders Datenbank <span class="sr-only">(current)</span></a>
                                    </li>
...
@section('content')
                ...
                <table class="table table-striped">
                        <thead>
                                <tr>
                                        ...
                                        <th scope="col">Offered Capacity Price [EUR/MW]</th>
                                        <th scope="col">Allocated Capacity [MW]</th>
                                        <th scope="col">Country</th>
                                        <th scope="col">Clearing Price [EUR/MW]</th>
                                </tr>
                        </thead>
                        <tbody>
                                @foreach($alltenders as $alltender)
                                <tr>
                                        <td>{{$alltender['DATE_FROM']}}</td>
                                        <td>{{$alltender['DATE_TO']}}</td>
                                        <td>{{$alltender['PRODUCT']}}</td>
                                        <td>{{$alltender['OFFERED_CAPACITY_PRICE']}}</td>
                                        <td>{{$alltender['ALLOCATED_CAPACITY']}}</td>
                                        <td>{{$alltender['COUNTRY']}}</td>
                                        <td>{{$alltender['SETTLEMENTCAPACITY_PRICE']}}</td>
                                </tr>
                                @endforeach
                        </tbody>
                </table>
        ...
        <div class="col-md-4">
                ...
                <div class="btn-group btn-group-md" role="group">
                    ...
                    <form action="{{ route('importatenders') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <a class="btn btn-secondary" href="{{ route('exportatenders') }}">Export Bedarfe</a>
